...
const useBoardEvent = (board: PlaitBoard, viewportContainerRef: React.RefObject<HTMLDivElement | null>) => {
    useEventListener(
        'scroll',
        (event) => {
    ...
    useEventListener(
        ...
        (event) => {
            ...
            if (event.metaKey || event.ctrlKey) {
                ...
                updateZoomFromCurrentViewport(board, newZoom, PlaitBoard.getMovingPointInBoard(board));
            }
        },
        { target: viewportContainerRef, passive: false }
    );
