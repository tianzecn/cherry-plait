import { BoardTransforms, distanceBetweenPointAndPoint, getPointBetween, MAX_ZOOM, MIN_ZOOM, PlaitBoard, Point } from '@plait/core';
...
export const withPinchZoom = (board: PlaitBoard) => {
    ...
    board.pointerMove = (event: PointerEvent) => {
        ...
        if (pointerRecords.length >= 2) {
            const record = pointerRecords.find((r) => r.pointerId === event.pointerId);
            if (record) {
                record.currentPoint = point;
                record.hasMoved = true;

                // 检查是否两个触控点都移动过
                if (pointerRecords.length === 2 && pointerRecords.every((r) => r.hasMoved)) {
                    ...
                    const v1 = [p1.currentPoint[0] - p1.lastPoint[0], p1.currentPoint[1] - p1.lastPoint[1]] as Point;
                    const v2 = [p2.currentPoint[0] - p2.lastPoint[0], p2.currentPoint[1] - p2.lastPoint[1]] as Point;
                    ...
                    if (isPinching) {
                        ...
                        newOrigination = [centerX - zoomCenterWidth / newZoom, centerY - zoomCenterHeight / newZoom] as Point;
                    }
    ...
    board.pointerUp = (event: PointerEvent) => {
        const index = pointerRecords.findIndex((r) => r.pointerId === event.pointerId);
        if (index !== -1) {
